...
Retorno: VOID
*/
void agregar_categoria(A_categoria arreglo[], int & dl_categoria){
    //Inserta elementos(codigo_categoria, descripcion)
    if(dl_categoria < DIM_FISICA){
        cout<<"Ingresar el codigo de la categoria: "<<flush;
        cin >>arreglo[dl_categoria].codigo_categoria;
        cout<<"Ingresar una descripcion de la categoria: "<<flush;
        getline(cin>>ws, arreglo[dl_categoria].descripcion);
    }
}
...
int submenu_1(char opcion){
    ...
    while(opcion != '0'){
        ...
        {
                int dl_categoria = 0;
                agregar_categoria(categoria, dl_categoria);
                break;
        }
        case '2':
                modificar_categoria();
